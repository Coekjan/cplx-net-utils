...
buf = []  # 临时存储配置命令的列表
dev_name: str | None = None  # 当前设备的名称
dev_nets = []  # 当前设备的网络列表
ospf_depth = 0
bgp_peers: set[str] = set()  # BGP 对等体的集合
mypre = "186"  # 预设前缀，用于替换 CIDR 地址中的前缀
telnet_port = None  # Telnet 连接的端口号
dev_open_mode = "w"  # 设备打开模式，写入或追加
asn = 1  # 自治系统号
vpn_no = 0  # VPN 号码
vpn_name = ""  # VPN 名称
is_rt = False  # 标志当前设备是否是路由器
port_cidr = dict()  # 端口与CIDR地址的对应关系字典
dev_addrs = set()  # 设备的IP地址集合
...
def nat_rtce(no, addr_group_if, permit):
    ...
    push(f"inter {interface}", "undo nat outbound 2001 address-group 1", "quit")
    ...
    push("undo nat address-group 1", f"nat address-group 1 {addr_st} {addr_ed}")
...
def vpn_inst(rd, *cmds):
    rd_imports = (
        tuple(filter(lambda cmd: cmd.split("=")[0] == "import", cmds))[0]
        .split("=")[1]
        .split(",")
    )
    rd_exports = (
        tuple(filter(lambda cmd: cmd.split("=")[0] == "export", cmds))[0]
        .split("=")[1]
        .split(",")
    )
